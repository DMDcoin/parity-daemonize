use libc::{
    c_int, close, dup2, fork, getpid, gid_t, ioctl, open, pipe, setgid, setsid, setuid, splice,
    uid_t, umask, FIONREAD, STDERR_FILENO, STDIN_FILENO, STDOUT_FILENO,
};
use log::{error, trace};
use mio::*;
use std::{
...
use super::unix_pipe::*;
use crate::{map_err, AsHandle, Error};
...
impl AsHandle for Handle {
    ...
    fn detach(&mut self) {
        let msg = ansi_term::Colour::Green
            .paint("Daemon started successfully, detaching ...\n")
            .to_string();
    ...
    fn detach_with_msg<T: AsRef<[u8]>>(&mut self, msg: T) {
        ...
        file.write_all(msg.as_ref()).expect(
            "Parent process won't exit until detach is called; \
...
pub fn daemonize<T: Into<PathBuf> + Sized>(pid_file: T) -> Result<Handle> {
    unsafe {
        ...
        map_err!(
            pipe(&mut chan[0] as *mut c_int),
            Error::Pipe(io::Error::last_os_error())
        )?;
        map_err!(
            pipe(&mut out_chan[0] as *mut c_int),
            Error::Pipe(io::Error::last_os_error())
        )?;
        map_err!(
            pipe(&mut err_chan[0] as *mut c_int),
            Error::Pipe(io::Error::last_os_error())
        )?;
        ...
        let pid_fd = map_err!(
            ...
            Error::OpenPidfile(io::Error::last_os_error())
        )?;
        ...
        // fork once
        let pid = map_err!(fork(), Error::Fork(io::Error::last_os_error()))?;

        if pid == 0 {
            // redirect stderr/stdout to out/err pipe
            // incase we get an error before forking
            map_err!(
                dup2(err_tx, STDERR_FILENO),
                Error::Dup2(io::Error::last_os_error())
            )?;
            map_err!(
                dup2(out_tx, STDOUT_FILENO),
                Error::Dup2(io::Error::last_os_error())
            )?;
            trace!(target: "daemonize", "created child Process! {}", getpid());

            set_current_dir("/").map_err(|_| Error::ChangeDirectory)?;
            set_sid()?;
            umask(0o027);
            // fork again
            let pid = map_err!(fork(), Error::Fork(io::Error::last_os_error()))?;
            ...
            .map_err(Error::RegisterationError)?;
                    ...
                    match event.token() {
                        STDOUT_READ_PIPE => {
                            ...
                            map_err!(
                                ...
                                Error::SpliceError(io::Error::last_os_error())
                            )?;
                        }
                        STDERR_READ_PIPE => {
...
unsafe fn set_sid() -> Result<()> {
    map_err!(setsid(), Error::DetachSession(io::Error::last_os_error()))?;
    Ok(())
}
...
unsafe fn get_pending_data_size(fd: RawFd) -> Result<usize> {
    ...
    map_err!(
        ...
        Error::Ioctl(io::Error::last_os_error())
    )?;
    Ok(size)
}
